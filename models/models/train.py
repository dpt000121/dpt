...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        # run the mdoel
        output, output2, output3, output4,ss= model(pre_img_var, post_img_var)
        ...
        loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + \
               BCEDiceLoss(output4, target_var)+0.5 * BCEDiceLoss(ss, target_var)
...
def train(args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        # run the model
        output, output2, output3, output4,ss = model(pre_img_var, post_img_var)  # 2 1 256 256
        # print(output.shape)
        # print(ss)
        # loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + \
        #        BCEDiceLoss(output4, target_var)
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/slr/docker_share/LEVIR-CD-256-split'
    elif args.file_root == 'BCDD':
        args.file_root = '/slr/docker_share/BCDD'
    elif args.file_root == 'SYSU':
        args.file_root = '/slr/docker_share/SYSU-CD'
    elif args.file_root == 'CDD':
        args.file_root = '/slr/docker_share/CDD-256-split'
    elif args.file_root == 'SVCD':
        args.file_root = '/slr/docker_share/SVCD'
    elif args.file_root == 'WHU':
        args.file_root = '/slr/docker_share/WHU-CD-224'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="WHU", help='Data directory | LEVIR | BCDD | SYSU | CDD | SVCD |WHU')
    ...
    parser.add_argument('--step_loss', type=int, default=150, help='Decrease learning rate after how many epochs')
