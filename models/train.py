...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        # run the mdoel
        output, output2= model(pre_img_var, post_img_var)
        ...
        #        BCEDiceLoss(output4, target_var)+0.4 * BCEDiceLoss(ss, target_var) + 0.4 * BCEDiceLoss(xx,target_var)
...
def train(args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        # run the model
        output, output2 = model(pre_img_var, post_img_var)  # 2 1 256 256

        loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var)
        # loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + \
        #        BCEDiceLoss(output4, target_var) + 0.4* BCEDiceLoss(ss, target_var) + 0.4 * BCEDiceLoss(xx,target_var)  # target:2 1 256 256
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/home/slr/mount/code/A2net备份/LEVIR-CD-256-split'
    elif args.file_root == 'BCDD':
        args.file_root = '/home/guan/Documents/Datasets/ChangeDetection/BCDD'
    elif args.file_root == 'SYSU':
        args.file_root = '/home/slr/mount/code/SYSU'
    elif args.file_root == 'WHU256':
        args.file_root = '/home/slr/mount/code/WHU_256split'
